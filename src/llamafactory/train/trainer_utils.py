...
@contextmanager
def maybe_disable_hf_zero3_init_for_ref_model():
    if os.environ.get("NPO_REF_DISABLE_ZERO3_INIT", "0").lower() not in {"1", "true", "yes"}:
        yield
        return

    weak_ref = getattr(hf_deepspeed, "_hf_deepspeed_config_weak_ref", None)
    ds_config = weak_ref() if weak_ref is not None else None
    if ds_config is None:
        yield
        return

    logger.info("Temporarily disabling HF DeepSpeed ZeRO-3 init while loading the frozen NPO reference model.")
    hf_deepspeed.unset_hf_deepspeed_config()
    try:
        yield
    finally:
        hf_deepspeed.set_hf_deepspeed_config(ds_config)
...
def create_ref_model(
    ...
    if finetuning_args.ref_model is not None:
        ...
        with maybe_disable_hf_zero3_init_for_ref_model():
...
def align_scheduler_with_optimizer_param_groups(scheduler: "torch.optim.lr_scheduler.LRScheduler"):
    """Keep HF schedulers compatible with DeepSpeed optimizer param-group merging.

    PyTorch 2.12 updates scheduler LRs with ``zip(..., strict=True)``. DeepSpeed
    ZeRO can expose fewer param groups than the AdamW optimizer HF used while
    constructing the scheduler, which makes the strict zip fail on step 1.  The
    schedules used here are constant/warmup schedules, so trimming or extending
    the scheduler metadata to the actual optimizer groups preserves the intended
    LR behavior.
    """
    def align_now() -> None:
        ...
        align_list("base_lrs")
        align_list("_last_lr")
        align_list("lr_lambdas")

    align_now()
    if getattr(scheduler, "_llamafactory_group_align_wrapped", False):
        return scheduler
    ...
    def safe_step(*args, **kwargs):
        ...
        try:
            ...
            if "zip() argument 2 is longer than argument 1" not in str(exc):
                raise
            align_now()
            return original_step(*args, **kwargs)

    scheduler.step = safe_step
    scheduler._llamafactory_group_align_wrapped = True
    return scheduler
